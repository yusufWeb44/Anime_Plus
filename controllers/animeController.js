...
const getFranchiseIds = async (Anime, AnimeRelation, Op, currentAnime) => {
  ...
  if (allVisited.length > 0) {
    // TEMPORARY: Bypass the word overlap filter completely to see if it's the root cause of the Render issue.
    // If names are in different languages (e.g. Arabic vs English), the overlap will always be 0.
    for (const c of allVisited) {
      safeIds.push(c);
    }
  }
