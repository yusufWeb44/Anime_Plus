...
// Fallback SMTP setup (for local dev if Resend is not set)
...
exports.sendResetPasswordEmail = async (email, username, resetUrl) => {
  const subject = "Reset Your Password - AnimePlus";
  const htmlContent = `
  ...
  // If Resend API key is available (Render Production), use Resend via HTTP API
  if (resend) {
    console.log("Using Resend API to send email...");
    const { data, error } = await resend.emails.send({
      from: 'AnimePlus <onboarding@resend.dev>', // You can change this to a verified domain later
      to: email,
      subject: subject,
      html: htmlContent,
    });
  ...
  // Otherwise, use Nodemailer (Local Development)
